...
/**
 * TRIÈDRE — Tarification LIVE Postes Canada
 * LIVE : obtient un jeton OAuth puis demande des tarifs réels liés au compte TRIÈDRE.
...
echo '<p class="muted">Tarification LIVE — consultation des tarifs uniquement, aucune étiquette, aucun achat.</p>';

if ($clientId === '' || $clientSecret === '' || $customerNumber === '') {
    echo '<h2 class="err">Configuration LIVE Postes Canada incomplète dans config.php.</h2></main></body></html>';
    exit;
}
...
</div><button type="submit">Obtenir les tarifs LIVE</button></form>';
...
echo '<pre>DEBUG — Type de contenu reçu : '.h($rateType).'</pre>';
echo '<pre>DEBUG — Longueur de la réponse : '.strlen((string)$rateResponse).' caractères</pre>';
